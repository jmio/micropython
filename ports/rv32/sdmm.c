...
#include "sdio.h"		/* Include device specific declareation file here */

#define DO_INIT()						/* Initialize port for MMC DO as input */
#define DO			(GPIOA_PIN & 0x01)	/* Test for MMC DO ('H':true, 'L':false) */

#define DI_INIT()	GPIOA_DIR |= 0x02	/* Initialize port for MMC DI as output */
#define DI_H()		GPIOA_OUT |= 0x02	/* Set MMC DI "high" */
#define DI_L()		GPIOA_OUT &= 0xFD	/* Set MMC DI "low" */

#define CK_INIT()	GPIOA_DIR |= 0x04	/* Initialize port for MMC SCLK as output */
#define CK_H()		GPIOA_OUT |= 0x04   /* Set MMC SCLK "high" */
#define	CK_L()		GPIOA_OUT &= 0xFB	/* Set MMC SCLK "low" */


#define CS_INIT()	GPIOA_DIR |= 0x08	/* Initialize port for MMC CS as output */
#define	CS_H()		GPIOA_OUT |= 0x08	/* Set MMC CS "high" */
#define CS_L()		GPIOA_OUT &= 0xF7	/* Set MMC CS "low" */
...
)
{
	...
	do {
		GPIOA_SPICNT = 0x08;
		while(GPIOA_SPICNT);
		*buff++ = GPIOA_SPIDAT;			/* Store a received byte */
	} while (--bc);
}
